...
for (let index = 0; index < argumentsAfterCommand.length; index += 1) {
	...
	if (argument === "--bundle") {
		...
		throw new Error(`Unknown argument "${argument}". Usage: [--bundle <journey>=<recording-id>] [--results-root <dir>]`);
	}
}
...
for (const [journey, recordingId] of pinnedBundles) {
	...
	if (!pinned) {
		throw new Error(`No passed ${journey} bundle with recording id "${recordingId}" under ${resultsRoot}.`);
	}
...
for (const [journey, plan] of Object.entries(exportPlan)) {
	...
	if (manifest.artifacts?.video && existsSync(join(bundleDir, manifest.artifacts.video))) {
...
writeFileSync(manifestPath, `${banner}export const siteMedia = ${JSON.stringify(siteMedia, null, "\t")} as const;\n`);

const oxfmt = spawnSync(
	process.platform === "win32" ? "pnpm.cmd" : "pnpm",
	["exec", "oxfmt", "--write", manifestPath],
	{ cwd: repositoryRoot, shell: process.platform === "win32", stdio: "inherit", windowsHide: true }
);
if (oxfmt.status !== 0) {
